...
<form id="cnslRegForm" method="POST" enctype="multipart/form-data">
		<table class="board-write-1s mgT40">
			...
			<tbody>
					...
					<tr>
						...
						<td>
							<input type="text" name="noticeTitle" id="inputTitle" />
						</td>
					</tr>
					<tr class="textarea">
						<th scope="col"><label for="InputTextarea">내용</label></th>
						<td>
							<textarea id="InputTextarea" name="noticeContent" cols="5" rows="1" placeholder="문의내용을 입력해주세요.(2000자 이내)" style="width:98%;height:280px;"></textarea>
							<label for="inputFile">
		...
		<div class="area1sButton pdT30">
			...
			<a href="${pageContext.request.contextPath }/cs/notice/viewall" id="cnslCancel" class="btnGray">취소</a>
			</div>		
...
if(navigator.appName=="Microsoft Internet Explorer"){
    ...
    $("#cnslSubmit").click(function() {        
            ...
            console.log(form);
            form.action = "${pageContext.request.contextPath }/cs/notice/write";
            form.submit();
    });
    ...
    $("#cnslCancel").click(function() {
        ...
        if (cnslLrgCate != "" || cnslMidCate != "" ||inqContLeng > 0 || InqContVal != ""||fileCk !=""){
            if(confirm("입력하신 내용이 저장되지 않습니다. 취소하시겠습니까?")){
                window.location.href= "${pageContext.request.contextPath }/cs/notice/viewall";
                return false;
            }
        }else{
            window.location.href= "${pageContext.request.contextPath }/cs/notice/viewall";
